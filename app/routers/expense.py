...
def add_Expense(payload:Create_Expense,db:Session = Depends(get_db),currentuser = Depends(get_current_user)):
    expense.add_Expense(payload,db)
    return "ok"

@router.get('/get_expense',response_model=Show_Expense,status_code=status.HTTP_200_OK)
def show_Expense(e_id:int,db:Session = Depends(get_db),currentuser = Depends(get_current_user)):
...
def show_User_Expense(u_id:int,db:Session = Depends(get_db),currentuser = Depends(get_current_user)):
    return expense.show_User_Expense(u_id,db)

@router.put('/update_expense',status_code=status.HTTP_202_ACCEPTED)
def update_Expense(payload:Update_Expense,db:Session = Depends(get_db),currentuser = Depends(get_current_user),e_id:int|None = None):
    return expense.update_Expense(payload,db,e_id)

@router.delete('/delete_expense',status_code=status.HTTP_200_OK)
def delete_Expense(e_id:int,db:Session = Depends(get_db),currentuser = Depends(get_current_user)):
