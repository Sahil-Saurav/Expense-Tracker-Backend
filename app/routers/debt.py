...
def create_Debt(payload:Create_Debt,db:Session = Depends(get_db),currentuser = Depends(get_current_user),creditor_id : int|None = None,debtor_id : int|None = None,creditor_email : str|None = None,debtor_email : str|None = None):
...
def show_Debt(d_id:int,db:Session = Depends(get_db),currentuser = Depends(get_current_user)):
...
def show_Debt_On_User(db:Session = Depends(get_db),currentuser = Depends(get_current_user),debtor_id:int|None = None,email:str|None = None):
...
def show_Credit_On_User(db:Session = Depends(get_db),currentuser = Depends(get_current_user),creditor_id:int|None = None,email:str|None = None):
    return debt.show_Credit_On_User(db,creditor_id,email)

@router.delete('/delete_debt',status_code=status.HTTP_200_OK)
def delete_Debt(d_id:int,db:Session = Depends(get_db),currentuser = Depends(get_current_user)):
