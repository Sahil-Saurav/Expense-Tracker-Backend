...
def get_User(db:Session = Depends(get_db),currentuser = Depends(get_current_user),id:int|None = None,email:str|None = None):
    return user.show_User(db,id,email)

@router.put('/update_user_details',status_code=status.HTTP_202_ACCEPTED)
def update_User(payload:Update_User,db:Session = Depends(get_db),currentuser = Depends(get_current_user),id:int|None = None,email:str|None = None):
    return user.update_User(payload,db,id,email)

@router.put('/change_password',status_code=status.HTTP_202_ACCEPTED)
def change_Password(payload:Change_Password,db:Session=Depends(get_db),currentuser = Depends(get_current_user),id:int|None = None,email:str|None = None):
    return user.change_Password(payload,db,id,email)

@router.put('/change_dob',status_code=status.HTTP_202_ACCEPTED)
def update_DOB(payload:Update_DOB,db:Session=Depends(get_db),currentuser = Depends(get_current_user),id:int|None = None,email:str|None = None):
    return user.update_DOB(payload,db,id,email)

@router.delete('/delete_user',status_code=status.HTTP_200_OK)
def delete_User(db:Session = Depends(get_db),currentuser = Depends(get_current_user),u_id:int|None = None,email:str|None = None):
